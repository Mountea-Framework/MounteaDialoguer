	...
	return (
		<div
			className={`
				...
				${selected || isPreviewActive ? 'border-primary ring-2 ring-primary/20' : 'border-blue-500'}
			`}
			title={tooltip}
		>
			{/* Input Handle */}
			...
			{/* Header */}
			<div className="bg-blue-500 text-white px-4 py-2 rounded-t-md flex items-center justify-between">
				...
				{data.decorators && data.decorators.length > 0 && (
					<div className="flex flex-wrap gap-1 pt-2 border-t">
						{data.decorators.map((decorator, idx) => (
							<div
								key={idx}
								className="flex items-center gap-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full text-xs"
								title={
